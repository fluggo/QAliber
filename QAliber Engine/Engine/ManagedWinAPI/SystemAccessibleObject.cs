...
namespace ManagedWinapi.Accessibility
{
	...
	public class SystemAccessibleObject
	{
		...
		/// <summary>
		/// Return the codepath to be used in QAliber Test Developer
		/// </summary>
		[Category("Common")]
		...
		/// <summary>
		/// Return the name by either rolestring or name
		/// </summary>
		[Browsable(false)]
		...
		/// </summary>
		[Browsable(false)]		  
		public List<SystemAccessibleObject> Children
		{
			get
			{
				// ID-referenced objects cannot have children
				if (childID != 0) return new List<SystemAccessibleObject>();
				...
				if (result != 0 && result != 1)
					return new List<SystemAccessibleObject>();
		...
		private static bool DeepEquals(IAccessible ia1, IAccessible ia2)
		{
			...
			if (sa1.Layout != sa2.Layout) return false;
